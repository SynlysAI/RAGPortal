type Status = 'pending' | 'processing' | 'success' | 'failed' | 'deleted'
...
const LABELS: Record<Status, { text: string; cls: string; icon: string }> = {
  ...
  deleted:    { text: '上游已删除', cls: 'bg-slate-100 text-slate-600', icon: '—' },
}
...
export default function StatusBadge({ status, tooltip }: Props) {
  ...
  const title = status === 'failed' || status === 'deleted'
    ? (tooltip || (status === 'deleted' ? '上游文档已删除' : '上传失败'))
    : undefined
  return (
    <span
